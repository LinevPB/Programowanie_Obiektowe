...
class Program
{
    ...
    static List<ClientTask2> ReadClients()
    {
        ...
        using (var csv = new CsvReader(reader, CultureInfo.InvariantCulture))
        {
            try
            {
                var records = csv.GetRecords<ClientTask>().ToList();
                return records;
            }
            catch (Exception ex)
            {
    ...
    static void WriteClients(List<ClientTask> clients)
    {
    ...
    {
        List<ClientTask> clients = ReadClients();

        ClientTask newClient = new ClientTask();
